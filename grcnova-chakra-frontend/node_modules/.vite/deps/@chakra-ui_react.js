import {
  ...
  toHex,
  transparentize
} from "./chunk-RUGNOEQG.js";
import {
  __assign,
  __rest,
  __spreadArray,
  require_prop_types
} from "./chunk-3TAQNHGG.js";
...
function getOwnerWindow(node2) {
  return getOwnerDocument(node2)?.defaultView ?? window;
}
...
var DescendantsManager = class {
  constructor() {
    ...
    __publicField2(this, "indexOf", (node2) => {
      if (!node2)
        return -1;
      return this.descendants.get(node2)?.index ?? -1;
    });
...
function useMenu(props = {}) {
  ...
  const refocus = (0, import_react92.useCallback)(() => {
    ...
    if (!shouldRefocus)
      return;
    const node2 = descendants.item(focusedIndex)?.node;
    node2?.focus({ preventScroll: !scrollIntoViewRef.current });
...
var variants14 = {
  ...
  exit: ({ offsetY, offsetX, transition: transition3, transitionEnd, reverse, delay }) => {
    ...
    return {
      opacity: 0,
      transition: transition3?.exit ?? withDelay.exit(TRANSITION_DEFAULTS.exit, delay),
      ...reverse ? { ...offset2, transitionEnd: transitionEnd?.exit } : { transitionEnd: { ...offset2, ...transitionEnd?.exit } }
    };
  }
};
var slideFadeConfig = {
